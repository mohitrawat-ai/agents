...
from slackbot.parse_alert import parse_alert  # 'slack' would collide with slack_sdk's deprecated shim
...
def _watch_run(proc: subprocess.Popen, slug: str, channel: str,
               thread_ts: str, client, stop_tailer: threading.Event) -> None:
...
def _handle_question(incident_dir: Path, question: str, channel: str,
                     anchor: str, client) -> None:
    ...
    proc = subprocess.run(
        [sys.executable, str(QA_PY), "--incident-dir", str(incident_dir),
         "--question", question, "--chart-dir", str(chart_dir)],
        capture_output=True, text=True, timeout=300, cwd=str(RCA_ROOT),
    )
    answer = proc.stdout.strip() or f"Q&A failed: {proc.stderr.strip()[:300]}"
    _say(client, channel, anchor, answer)
    chart = chart_dir / "chart.png"
    if chart.exists():
        client.files_upload_v2(channel=channel, thread_ts=anchor,
                               file=str(chart), title="chart.png")
...
def on_mention(event, client, logger):
    ...
    with _lock:
        ...
        if _active_run is not None:
            ...
            return
        _recent[parsed["dedup_key"]] = {"slug": parsed["slug"], "ts": now,
                                        "thread": anchor}
